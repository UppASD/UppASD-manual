...
See :doc:`input-keywords-averages` for detailed measurement configuration.
...
- ``Temp``: Temperature in Kelvin (see :doc:`input-keywords-simulation`)
- ``mcnstep``: Number of Monte Carlo sweeps (see :doc:`input-keywords-simulation`)
- ``Mensemble``: Number of ensemble replicas for parallel tempering (see :doc:`input-keywords-simulation`)
- ``do_avrg``, ``avrg_step``: Average magnetization measurements (see :doc:`input-keywords-averages`)
- ``do_cumu``: Cumulant measurements (automatic for MC; see :doc:`input-keywords-averages`)
- ``plotenergy``: Energy component output for specific heat (see :doc:`input-keywords-observables`)
