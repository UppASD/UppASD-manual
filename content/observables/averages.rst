...
skyrmion counting is enabled (see :doc:`topology`).
...
- ``Temp``: Temperature for susceptibility and specific heat calculations (see :doc:`../input/simulation`)
- ``delta_t``: Timestep for real-time measurements (see :doc:`../input/simulation`)
- ``Mensemble``: Number of ensembles for statistical averaging (see :doc:`../input/simulation`)
- ``plotenergy``: Enables energy measurements for specific heat (see :doc:`../input/observables`)
- ``do_ralloy``: Enables random alloy simulations for chemical projections (see :doc:`../input/random-alloys`)
- ``compensate_drift``: Activates QHB thermostat with temperature rescaling (see :doc:`../input/simulation`)

Output files are always named with the simulation identifier ``simid`` (see :doc:`../input/system`).
